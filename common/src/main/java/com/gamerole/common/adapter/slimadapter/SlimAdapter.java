...
import com.gamerole.common.adapter.slimadapter.diff.DefaultDiffCallback;
import com.gamerole.common.adapter.slimadapter.diff.SlimDiffUtil;
...
public class SlimAdapter<D> extends AbstractSlimAdapter implements IDataAdapter<List<D>> {
    ...
    public SlimAdapter updateData(List<D> data) {
    ...
    @Override
    public void notifyDataChanged(List<D> t, boolean isRefresh) {
        if (!isRefresh) {
            if (t != null && t.size() > 0) {
                data.addAll(t);
                updateData(data);
    ...
    public SlimAdapter registerDefault(final int layoutRes, final SlimInjector slimInjector) {
        defaultCreator = parent -> new SlimTypeViewHolder(parent, layoutRes) {
            @Override
    ...
    public <T> SlimAdapter register(final int layoutRes, final SlimInjector<T> slimInjector) {
        ...
        creators.put(type, parent -> new SlimTypeViewHolder<T>(parent, layoutRes) {
            @Override
